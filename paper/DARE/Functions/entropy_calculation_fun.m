function[entropy] = entropy_calculation_fun(T,specie,Ru,MW_specie)
    ...
    if T >= 1000
        entropy = R*(Dsh(1,1)*log(T) + Dsh(1,2)*T + Dsh(1,3)*(T^2)/2 + Dsh(1,4)*(T^3)/3 + Dsh(1,5)*(T^4)/4 + Dsh(2,2));
    else
        entropy = R*(Dsh(2,3)*log(T) + Dsh(2,4)*T + Dsh(2,5)*(T^2)/2 + Dsh(3,1)*(T^3)/3 + Dsh(3,2)*(T^4)/4 + Dsh(3,4));
    end
end