...
if choice == 1 && area_profile_comb_nozzle(end,1) < Comb_Length + nozzCond{2}
    error('The length info given in the area profile is not sufficient, please provide the area profile along with the full-lenght, update nozzle and combustor lengths, or select pre-defined expansion ratio option');
end
...
if strcmp( engineMode, 'RAM' )
    ...
    elseif choice == 2 && nozzle_key == true
        ...
        fprintf('Final solution: Exit Mach Number=%14.6e, Nozzle Exit Area=%14.6e [m^2], Nozzle Length=%14.6e [m]', Mach_Nozzle_Exit, Area_Nozzle_Exit, Nozzle_Length); % Calculated nozzle parameters with a pre-defined expansion ratio
        performance_key = true;
    end
