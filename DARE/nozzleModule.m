...
options = CVodeSetOptions('RelTol',1.e-6,... % setting for the relative tolerance
    'AbsTol',[1.e-6; 1.e-6; 1.e-6; 1.e-6; 1.e-6; 1.e-9; 1.e-9; 1.e-9; 1.e-9; 1.e-9; 1.e-9; 1.e-9; 1.e-9; 1.e-9; 1.e-9; ...
    1.e-9; 1.e-9; 1.e-9; 1.e-6; 1.e-4],... % setting for the absolute tolerance
    ...
    if choice == 1 && nozzle_key == true
        while iout < imax
            ...
            if Mach_Mixture <= 1
                ...
                fprintf("Local Mach number decreased to the unity in the nozzle of the ramjet engine. Please reduce the injected " + ...
                    "amount of fuel or update the design parameters given above.")
                break;
            end
...
elseif strcmp( engineMode, 'SCRAM' )
    ...
    if choice == 1 && nozzle_key == true
        while iout < imax
            ...
            if Mach_Mixture <= 1
                performance_key = false;
                fprintf("Local Mach number decreased to the unity in the scramjet engine duct. Please reduce the injected amount of fuel or" + ...
                    " update the design parameters given above.")
                break;
            end
            if x > Nozzle_Length + Comb_Length
                performance_key = true;
            ...
            fprintf(['solution = [ %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e  %14.6e ' ...
                ' %14.6e  %14.6e  %14.6e %14.6e %14.6e]\n\n'], y(1), y(2), y(3), y(4), y(5), y(6), y(7), y(8), y(9), y(10), y(11), y(12), y(13), y(14), y(15), ...
                y(16), y(17), y(18), y(19), y(20)); % Calculated parameters in the numerical solution
            if(status == 0)
                iout = iout+1; % increasing the number of iteration
            end
            xout = Nozzle_Length + Comb_Length;
    ...
    elseif choice == 2 && nozzle_key == true
        ...
        Nozzle_Area_Diameter = sqrt(4 * Area_Nozzle_Exit / 3.14);
        Nozzle_Length = (Nozzle_Area_Diameter - Combustor_Area_Diameter) / 2 / tand(nozzCond{4});
        fprintf('Final solution: Exit Mach Number=%14.6e, Nozzle Exit Area=%14.6e [m^2], Nozzle Length=%14.6e [m]', Mach_Nozzle_Exit, ...
            Area_Nozzle_Exit, Nozzle_Length); % Calculated nozzle parameters for perfect expansion
    end
end
%% Record output
nozzOut{1} = choice;
